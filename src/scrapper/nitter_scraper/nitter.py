...
class Nitter(DockerBase):
    ...
    def get_tweets_with_query(self, query: str, pages: int = 25):
        return get_tweets_using_query(
            query=query, 
            pages=pages, 
            address=self.address
        )
    
    def get_replies_for_tweet(self, path: str, pages: int = 25):
        return get_replies_for_tweet(
            path=path,
            pages=pages,
            address=self.address
        )
